...
namespace Escape.Renderer.Shader {
	...
	public static class IShaderArrayData {
		...
		public unsafe static IShaderArrayData<T> Create<T>(IPlatform platform, ShaderProgram program, string name, uint binding, T[]? data, uint? size = null) {
			uint realSize = size ?? (uint) ((data?.Length ?? 0) * sizeof(T));
			
			return platform switch {
				GLPlatform glPlatform => new GLShaderArrayData<T>(glPlatform, program, name, binding, data, realSize),
